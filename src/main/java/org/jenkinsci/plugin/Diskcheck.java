...
//   //     this.buildSteps = buildstep;
    ...
    //   if (PluginImpl.getInstance().isDiskrecyclerenabled() )
     //   {  
        ...
        {
       // 	System.out.println("I am in in the loop looking");
            build.getWorkspace().mkdirs();
        }
        
      
 //  	FilePath File1 = build.getWorkspace();
	Node node1 = build.getBuiltOn();
//	FilePath nodep =node1.getRootPath();
    ...
    long  size = DiskSpaceMonitor.DESCRIPTOR.get(Comp).size;
  	...
  	// log.println("node file workspace is" +  File1);
  	//  log.println("node file workspace is" + nodep);
    	 ...
    	 */
          CommandInterpreter runscript;
    		if (launcher.isUnix())
    			...
    			runscript=new BatchFile(mycommand);
  //		AbstractBuild build1 = build;
   ...
   Result result= runscript.perform(build, launcher, listener) ? Result.SUCCESS : Result.FAILURE;
        ...
        log.println("Running Prebuild steps");
    	System.out.println("build.getBuiltOnStr() (Built on Slave, if empty it is built on master): "+build.getBuiltOnStr());
		System.out.println("build.getDescription(): "+build.getDescription());
		System.out.println("build.getDisplayName(): "+build.getDisplayName());
		System.out.println("build.getDuration(): "+build.getDuration());
		System.out.println("build.getStartTimeInMillis(): "+build.getStartTimeInMillis());
		System.out.println("build.getProject().getDisplayName(): "+ build.getProject().getDisplayName());
		System.out.println("build.getProject().getFullDisplayName(): "+ build.getProject().getFullDisplayName());
		System.out.println("build.getProject().getFullName(): "+build.getProject().getFullName());
		System.out.println("build.getUrl(): "+build.getUrl());
    	
		
	
	 // log.println("node file workspace is File1");
	 if (roundedSize < SpaceThreshold && !(PluginImpl.getInstance().isDiskrecyclerenabled()))
	  {
    ...
    public static final class DescriptorImpl extends Descriptor<BuildWrapper> {
          ...
          /**
           ...
           public String getDisplayName() {
                    // TODO localization
                    return "Check my Disk Usage Fail if Low ";
            }
